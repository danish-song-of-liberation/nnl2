...
void init_system() {   
	...
	INIT_BACKEND(sigmoid, sigmoid_backends); 
	INIT_BACKEND(tanhinplace, tanhinplace_backends); 
	INIT_BACKEND(nnl2_tanh, tanh_backends); 
	INIT_BACKEND(nnl2_concat, concat_backends); 
	INIT_BACKEND(randn, randn_backends); 
	INIT_BACKEND(xavier, xavier_backends); 
	INIT_BACKEND(transposeinplace, transposeinplace_backends); 
	INIT_BACKEND(transpose, transpose_backends); 
	INIT_BACKEND(nnl2_sum, sum_backends); 
	INIT_BACKEND(l2norm, l2norm_backends); 
	INIT_BACKEND(nnl2_copy, copy_backends); 	
	...
	INIT_BACKEND(axpy_inplace, axpy_inplace_backends);
	INIT_BACKEND(axpy, axpy_backends);

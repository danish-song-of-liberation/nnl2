...
/** @file nnl2_ad_add_broadcasting_inplace.h
 ** @date 2025
 ** @copyright MIT
 ** @brief In-place broadcasting addition operation for AD tensors
 **/
 
/** @brief 
 * Applies in-place broadcasting addition operation to AD tensors
 *
 ** @param summand 
 * Pointer to the AD tensor to modify in-place (receives the result)
 *
 ** @param addend 
 * Pointer to the AD tensor to add to the summand (broadcasted if needed)
 *
 ** @param retain_graph 
 ...
 * nnl2_ad_add_broadcasting_inplace(tensor1, tensor2, false); // tensor1 += tensor2 (with broadcasting)
 ** @endcode
 **
...
void nnl2_ad_add_broadcasting_inplace(nnl2_ad_tensor* summand, nnl2_ad_tensor* addend, bool retain_graph) {
	...
	// Safety checks
		...
		if(!summand) {
			NNL2_ERROR("summand is NULL (in function nnl2_ad_add_broadcasting_inplace)"); 
		}
		
		if(!addend) {
			NNL2_ERROR("addend is NULL (in function nnl2_ad_add_broadcasting_inplace)"); 
		}
		
		#if NNL2_SAFETY_MODE >= NNL2_SAFETY_MODE_MAX
			if(!summand->data) {
				NNL2_ERROR("summand->data is NULL (in function nnl2_ad_add_broadcasting_inplace)"); 
			}
			
			if(!addend->data) {
				NNL2_ERROR("addend->data is NULL (in function nnl2_ad_add_broadcasting_inplace)"); 
			}
		#endif
	#endif
	...
	// Apply in-place broadcasting addition operation: summand += addend (with broadcasting)

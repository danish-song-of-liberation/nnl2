...
/** @file nnl2_ad_max_broadcasting_inplace.h
 ** @date 2025
 ** @copyright MIT
 ** @brief In-place broadcasting maximum operation for AD tensors
 **/
 
/** @brief 
 * Applies in-place broadcasting element-wise maximum operation to AD tensors
 *
 ** @param tensor_a 
 ...
 * nnl2_ad_max_broadcasting_inplace(tensor1, tensor2, false); // tensor1 = max(tensor1, tensor2) (with broadcasting)
 ** @endcode
 **
 ** @see NNL2_AD_INPLACE_FATAL
 ** @see NNL2_ERROR
 **/
void nnl2_ad_max_broadcasting_inplace(nnl2_ad_tensor* tensor_a, nnl2_ad_tensor* tensor_b, bool retain_graph) {
	...
	// Safety checks
		...
		if(!tensor_a) {
			NNL2_ERROR("tensor_a is NULL (in function nnl2_ad_max_broadcasting_inplace)"); 
		}
		
		if(!tensor_b) {
			NNL2_ERROR("tensor_b is NULL (in function nnl2_ad_max_broadcasting_inplace)"); 
		}
		
		#if NNL2_SAFETY_MODE >= NNL2_SAFETY_MODE_MAX
			if(!tensor_a->data) {
				NNL2_ERROR("tensor_a->data is NULL (in function nnl2_ad_max_broadcasting_inplace)"); 
			}
			
			if(!tensor_b->data) {
				NNL2_ERROR("tensor_b->data is NULL (in function nnl2_ad_max_broadcasting_inplace)"); 
			}
		#endif
	#endif
	...
	// Apply in-place broadcasting element-wise maximum operation: tensor_a = max(tensor_a, tensor_b) (with broadcasting)

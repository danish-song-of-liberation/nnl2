...
/** @file nnl2_ad_max_correspondence.h
 ** @brief AD implementation for maximum with correspondence operation
 ** @date 2025
 ** @copyright MIT
 **/

/** @brief 
 * Reverse mode backward pass for maximum with correspondence operation
 *
 ** @param tensor
 * The output tensor from max_correspondence operation that needs gradient computation
 *
 ** @details
 * Maximum with correspondence operation: result = max(tensor, threshold)
 ...
 *
 ** @see nnl2_ad_max_correspondence()
 ** @see nnl2_ad_reverse_derivative_max_correspondence()
 **/	
static void nnl2_ad_reverse_backward_max_correspondence(nnl2_ad_tensor* tensor) {
	...
	// Safety checks
		...
		NNL2_CHECK_NULL_IF_ERR_RETURN(tensor, "In function nnl2_ad_reverse_backward_max_correspondence, passed AD tensor is NULL");
		NNL2_CHECK_NULL_IF_ERR_RETURN(tensor->data, "In function nnl2_ad_reverse_backward_max_correspondence, passed AD tensor data is NULL");
		NNL2_CHECK_NULL_IF_ERR_RETURN(tensor->roots[0], "In function nnl2_ad_reverse_backward_max_correspondence, root tensor is NULL");
		NNL2_CHECK_NULL_IF_ERR_RETURN(tensor->extra_correspondence, "In function nnl2_ad_reverse_backward_max_correspondence, extra_correspondence is NULL");
	#endif
	
	// Compute the derivative of max_correspondence operation and propagate to root tensor
...
/** @brief 
 * Create an automatic differentiation tensor for maximum with correspondence operation
 *
 ** @param tensor 
 * Input tensor (first operand in max(tensor, threshold))
 *
 ** @param threshold 
 ...
 * New AD tensor containing element-wise maximum of tensor and threshold, or NULL on failure
 *
 ** @exception NNL2Error
 ...
 *
 ** @exception NNL2Error
 * Returns NULL if max_maxf operation fails on input data
 *
 ** @exception NNL2Error
 ...
 **/
nnl2_ad_tensor* nnl2_ad_max_correspondence(nnl2_ad_tensor* tensor, void* threshold, nnl2_ad_mode ad_mode, bool track_graph) {
	#if NNL2_DEBUG_MODE >= NNL2_DEBUG_MODE_VERBOSE
		NNL2_FUNC_ENTER();
	#endif
	...
	// Basic null checks for input parameters
		...
		NNL2_CHECK_NULL_IF_ERR_RETURN_VAL(tensor, "In function nnl2_ad_max_correspondence, passed AD tensor is NULL", NULL);
		NNL2_CHECK_NULL_IF_ERR_RETURN_VAL(threshold, "In function nnl2_ad_max_correspondence, threshold is NULL", NULL);
	#endif
	
	// Comprehensive null checks for tensor components
	#if NNL2_SAFETY_MODE >= NNL2_SAFETY_MODE_FULL
		NNL2_CHECK_NULL_IF_ERR_RETURN_VAL(tensor->data, "In function nnl2_ad_max_correspondence, passed AD tensor data is NULL", NULL);
		NNL2_CHECK_NULL_IF_ERR_RETURN_VAL(tensor->data->shape, "In function nnl2_ad_max_correspondence, passed AD tensor shape is NULL", NULL);
	#endif
	...
	// Compute maximum with correspondence: max(tensor, threshold)
    result->data = max_maxf(tensor->data, threshold);
	if(!result->data) {
		NNL2_ERROR("In function nnl2_ad_max_correspondence, failed to compute maximum with correspondence");
		free(result);
		return NULL;
	}
	...
	if(!result->grad) {
		NNL2_ERROR("In function nnl2_ad_max_correspondence, failed to allocate nnl2 empty tensor for gradient using nnl2_empty function");
	...
	if(track_graph) {
		...
		switch(ad_mode) {
			case nnl2_ad_reverse_mode: 
				result->backward_fn = nnl2_ad_reverse_backward_max_correspondence;  
				break;

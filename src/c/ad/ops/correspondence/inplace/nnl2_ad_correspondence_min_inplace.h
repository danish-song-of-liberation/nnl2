...
void nnl2_ad_reverse_backward_min_minf_inplace(nnl2_ad_tensor* tensor) {
    nnl2_ad_reverse_derivative_min_minf_inplace(tensor, tensor->roots[0], tensor->extra_correspondence);
}   

void nnl2_ad_min_minf_inplace(nnl2_ad_tensor* tensor, void* threshold, nnl2_ad_mode ad_mode) {
    ...
    switch(ad_mode) {
        case nnl2_ad_reverse_mode: tensor->backward_fn = nnl2_ad_reverse_backward_min_minf_inplace; break;

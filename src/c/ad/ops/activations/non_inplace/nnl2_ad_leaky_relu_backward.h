...
void nnl2_ad_reverse_derivative_leakyrelu(nnl2_ad_tensor* out_tensor, nnl2_ad_tensor* a, nnl2_float32 alpha) {
    ...
    switch (dtype) {
        case FLOAT64: {
            ...
            for (size_t i = 0; i < numel; i++) ag[i] += (ad[i] > 0.0) ? gd[i] : (nnl2_float64)(alpha * gd[i]);
            break;
        }
        ...
        case FLOAT32: {
            ...
            for (size_t i = 0; i < numel; i++) ag[i] += (ad[i] > 0.0f) ? gd[i] : (alpha * gd[i]);
            break;
        }
        ...
        case INT32: {
            ...
            nnl2_float32* gd = (nnl2_float32*)out_tensor->grad->data;
            nnl2_float32* ag = (nnl2_float32*)a->grad->data;
            for (size_t i = 0; i < numel; i++) ag[i] += (ad[i] > 0) ? gd[i] : (alpha * gd[i]);
            break;
        }

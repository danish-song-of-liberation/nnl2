#ifndef NNL2_AD_TRANSPOSE_INPLACE_H
#define NNL2_AD_TRANSPOSE_INPLACE_H

// NNL2

/** @file nnl2_ad_transpose_inplace.h  
 ** @date 2025
 ** @copyright MIT
 ** @brief In-place transpose operation for AD tensors
 **/
 
/** @brief 
 * Applies in-place transpose operation to an AD tensor 
 *
 ** @param ad_tensor 
 ...
 * nnl2_ad_inplace_transpose(tensor, false, true); // Full data reordering
 * // tensor shape is now [2, 3] with data physically rearranged
 ** @endcode
 **
 ** @see NNL2_AD_INPLACE_FATAL
 ** @see NNL2_ERROR
 **/
void nnl2_ad_inplace_transpose(nnl2_ad_tensor* ad_tensor, bool retain_graph, bool force) {
	...
	// Safety checks
		...
		if(!ad_tensor) {
			NNL2_ERROR("ad_tensor is NULL (in function nnl2_ad_inplace_transpose)"); 
		}
		
		#if NNL2_SAFETY_MODE >= NNL2_SAFETY_MODE_MAX
			if(!ad_tensor->data) {
				NNL2_ERROR("ad_tensor->data is NULL (in function nnl2_ad_inplace_transpose)"); 
			}
			
			if(!ad_tensor->data->shape) {
				NNL2_ERROR("ad_tensor->data->shape is NULL (in function nnl2_ad_inplace_transpose)"); 
			}
		#endif
	#endif
	...
	// Apply in-place transpose operation to ad tensor data
    transposeinplace(ad_tensor->data, force);
	
	// Also transpose the gradient tensor if it exists to maintain shape consistency
	if(ad_tensor->grad) {
		transposeinplace(ad_tensor->grad, force);
	}
...
#endif /** NNL2_AD_TRANSPOSE_INPLACE_H **/

...
/** @file nnl2_ad_neg.h
 ** @brief AD implementation for negation operation
 ** @date 2025
 ** @copyright MIT
 **/

/** @brief 
 * Reverse mode backward pass for negation operation
 *
 ** @param tensor
 * The output tensor from neg operation that needs gradient computation
 *
 ** @details
 ...
 *
 ** @see nnl2_ad_neg()
 ** @see nnl2_ad_reverse_derivative_neg()
 **/	
static void nnl2_ad_reverse_backward_neg(nnl2_ad_tensor* tensor) {
	...
	// Safety checks
		...
		NNL2_CHECK_NULL_IF_ERR_RETURN(tensor, "In function nnl2_ad_reverse_backward_neg, passed AD tensor is NULL");
		NNL2_CHECK_NULL_IF_ERR_RETURN(tensor->data, "In function nnl2_ad_reverse_backward_neg, passed AD tensor data is NULL");
		NNL2_CHECK_NULL_IF_ERR_RETURN(tensor->roots[0], "In function nnl2_ad_reverse_backward_neg, root tensor is NULL");
	#endif
	
	// Compute the derivative of neg operation and propagate to root tensor
...
/** @brief 
 * Create an automatic differentiation tensor for negation operation
 *
 ** @param ad_tensor 
 * Input tensor
 *
 ...
 * New AD tensor containing negated values, or NULL on failure
 *
 ** @exception NNL2Error
 ...
 *
 ** @exception NNL2Error
 * Returns NULL if nnl2_neg operation fails on input data
 *
 ** @exception NNL2Error
 ...
 **/
nnl2_ad_tensor* nnl2_ad_neg(nnl2_ad_tensor* ad_tensor, nnl2_ad_mode ad_mode, bool track_graph) {
	#if NNL2_DEBUG_MODE >= NNL2_DEBUG_MODE_VERBOSE
		NNL2_FUNC_ENTER();
	#endif
	...
	// Basic null check for input tensor
		...
		NNL2_CHECK_NULL_IF_ERR_RETURN_VAL(ad_tensor, "In function nnl2_ad_neg, passed AD tensor is NULL", NULL);
	#endif
	
	// Comprehensive null checks for tensor components
	#if NNL2_SAFETY_MODE >= NNL2_SAFETY_MODE_FULL
		NNL2_CHECK_NULL_IF_ERR_RETURN_VAL(ad_tensor->data->shape, "In function nnl2_ad_neg, passed AD tensor shape is NULL", NULL);
		NNL2_CHECK_NULL_IF_ERR_RETURN_VAL(ad_tensor->data, "In function nnl2_ad_neg, passed AD tensor data is NULL", NULL);
	#endif
	...
	if(!result->data) {
		NNL2_ERROR("In function nnl2_ad_neg, failed to apply negation to the tensor data");
		free(result);
		return NULL;
	}
	...
	if(!result->grad) {
		NNL2_ERROR("In function nnl2_ad_neg, failed to allocate nnl2 empty tensor for gradient using nnl2_empty function");

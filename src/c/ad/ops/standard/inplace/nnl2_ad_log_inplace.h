...
void nnl2_ad_reverse_backward_inplace_log(nnl2_ad_tensor* tensor) {
    nnl2_ad_reverse_derivative_inplace_log(tensor, tensor->roots[0]);
}   

void nnl2_ad_inplace_log(nnl2_ad_tensor* ad_tensor, nnl2_ad_mode ad_mode) {
    ...
    switch(ad_mode) {
        case nnl2_ad_reverse_mode: ad_tensor->backward_fn = nnl2_ad_reverse_backward_inplace_log; break;

...
/** @file nnl2_ad_log_inplace.h
 ** @date 2025
 ** @copyright MIT
 ** @brief In-place natural logarithm operation for AD tensors
 **/
 
/** @brief 
 * Applies in-place natural logarithm operation to an AD tensor 
 *
 ** @param ad_tensor 
 ...
 * nnl2_ad_inplace_log(tensor, false); // No graph retention needed
 ** @endcode
 **
...
void nnl2_ad_inplace_log(nnl2_ad_tensor* ad_tensor, bool retain_graph) {
	...
	// Safety checks
		...
		if(!ad_tensor) {
			NNL2_ERROR("ad_tensor is NULL (in function nnl2_ad_inplace_log)"); 
		}
		
		#if NNL2_SAFETY_MODE >= NNL2_SAFETY_MODE_MAX
			if(!ad_tensor->data) {
				NNL2_ERROR("ad_tensor->data is NULL (in function nnl2_ad_inplace_log)"); 
			}
		#endif
	#endif
	...
	// Apply in-place natural logarithm operation to ad tensor data

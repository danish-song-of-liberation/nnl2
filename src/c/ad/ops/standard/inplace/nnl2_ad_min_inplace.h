...
/** @file nnl2_ad_min_inplace.h
 ** @date 2025
 ** @copyright MIT
 ** @brief In-place minimum operation for AD tensors
 **/
 
/** @brief 
 * Applies in-place element-wise minimum operation to AD tensors
 *
 ** @param result 
 * Pointer to the AD tensor to modify in-place (first operand and result)
 *
 ** @param operand 
 * Pointer to the AD tensor to compare with
 *
 ** @param retain_graph 
 ...
 * nnl2_ad_min_inplace(tensor1, tensor2, false); // tensor1 = min(tensor1, tensor2)
 ** @endcode
 **
...
void nnl2_ad_min_inplace(nnl2_ad_tensor* result, nnl2_ad_tensor* operand, bool retain_graph) {
	...
	// Safety checks
		...
		if(!result) {
			NNL2_ERROR("result is NULL (in function nnl2_ad_min_inplace)"); 
		}
		if(!operand) {
			NNL2_ERROR("operand is NULL (in function nnl2_ad_min_inplace)"); 
		}
		
		#if NNL2_SAFETY_MODE >= NNL2_SAFETY_MODE_MAX
			if(!result->data) {
				NNL2_ERROR("result->data is NULL (in function nnl2_ad_min_inplace)"); 
			}
			if(!operand->data) {
				NNL2_ERROR("operand->data is NULL (in function nnl2_ad_min_inplace)"); 
			}
		#endif
	#endif
	...
	// Apply in-place element-wise minimum operation: result = min(result, operand)

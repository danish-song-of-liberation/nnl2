...
/** @file nnl2_ad_max_inplace.h
 ** @date 2025
 ** @copyright MIT
 ** @brief In-place maximum operation for AD tensors
 **/
 
/** @brief 
 * Applies in-place element-wise maximum operation to AD tensors
 *
 ** @param result 
 ...
 * nnl2_ad_max_inplace(tensor1, tensor2, false); // tensor1 = max(tensor1, tensor2)
 ** @endcode
 **
...
void nnl2_ad_max_inplace(nnl2_ad_tensor* result, nnl2_ad_tensor* operand, bool retain_graph) {
	...
	// Safety checks
		...
		if(!result) {
			NNL2_ERROR("result is NULL (in function nnl2_ad_max_inplace)"); 
		}
		if(!operand) {
			NNL2_ERROR("operand is NULL (in function nnl2_ad_max_inplace)"); 
		}
		
		#if NNL2_SAFETY_MODE >= NNL2_SAFETY_MODE_MAX
			if(!result->data) {
				NNL2_ERROR("result->data is NULL (in function nnl2_ad_max_inplace)"); 
			}
			if(!operand->data) {
				NNL2_ERROR("operand->data is NULL (in function nnl2_ad_max_inplace)"); 
			}
		#endif
	#endif
	...
	// Apply in-place element-wise maximum operation: result = max(result, operand)

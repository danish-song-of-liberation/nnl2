	...
	switch(tensor->dtype) {
		...
		case INT64: {
			...
			for(size_t i = 0; i < total_elems; i++) 
				cast_data_result[i] = cast_data_original[i] + increment;
			break;
		}
...
/** @brief
 * Worker function for parallel int64 scalar addition with increment
 * 
 ** @param arg 
 * Pointer to addincf_non_inplace_ptask structure containing thread parameters
 *
 ...
 * NULL (for pthread API compatibility)
 */
void* nnl2_own_padd_incf_int64_non_inplace(void* arg);
...
nnl2_tensor* nnl2_own_add_incf(nnl2_tensor* tensor, void* inc) {
    ...
    switch(dtype) {
		...
		case INT64: {
			int64_t inc_val = *((int64_t*)inc);
			for (size_t i = 0; i < num_threads; i++) {
				tasks[i].dtype = dtype;
				tasks[i].aligned_tensor = is_aligned_tensor;
				tasks[i].aligned_result = is_aligned_result;
				tasks[i].increment.int64_inc = inc_val;
			}
			break;
		}
    ...
    for (size_t i = 0; i < num_threads; i++) {
        ...
        switch(dtype) {
			...
			case INT64:   worker_func = nnl2_own_padd_incf_int64_non_inplace;   break;
...
void* nnl2_own_padd_incf_int64_non_inplace(void* arg) {
    ...
    if(task->aligned_tensor && task->aligned_result) {
        for(; i + 3 < end; i += 4) {
            ...
            __m256i v_result = _mm256_add_epi64(v_tensor, v_increment);

 ...
 */
void naive_min_minf_inplace(nnl2_tensor* tensor, void* threshold) {
    ...
    switch(tensor->dtype) {
		...
		case INT64: {
			...
			int64_t min_val = *((int64_t*)threshold);
			for(size_t i = 0; i < total_elems; i++) cast_data[i] = MIN(cast_data[i], min_val);
			break;
		}
...
/** @brief
 * Threshold for enabling parallel execution of in-place element-wise minimum operation
 */
#define NNL2_MIN_MINF_INPLACE_PARALLEL_THRESHOLD 1000000

/** @brief
 * Worker function for parallel double precision in-place element-wise minimum operation
 * 
 ** @param arg 
 * Pointer to min_minf_inplace_ptask structure containing thread parameters
 *
 ** @return 
 ...
 */
void* nnl2_own_pmin_minf_inplace_float64(void* arg);

/** @brief
 * Worker function for parallel single precision in-place element-wise minimum operation
 * 
 ** @param arg 
 * Pointer to min_minf_inplace_ptask structure containing thread parameters  
 * 
 ...
 * NULL (for pthread API compatibility)
 * 
 ** @see nnl2_own_pmin_minf_inplace_float64
 **/
void* nnl2_own_pmin_minf_inplace_float32(void* arg);

/** @brief
 * Worker function for parallel int64 in-place element-wise minimum operation
 * 
 ** @param arg 
 * Pointer to min_minf_inplace_ptask structure containing thread parameters
 *
 ...
 ** @see nnl2_own_pmin_minf_inplace_float64
 **/
void* nnl2_own_pmin_minf_inplace_int64(void* arg);

/** @brief
 * Worker function for parallel integer in-place element-wise minimum operation
 * 
 ** @param arg 
 * Pointer to min_minf_inplace_ptask structure containing thread parameters
 *
 ...
 ** @see nnl2_own_pmin_minf_inplace_float64
 **/
void* nnl2_own_pmin_minf_inplace_int32(void* arg);
 ...
 */
void nnl2_own_min_minf_inplace(nnl2_tensor* tensor, void* threshold) {
    #if NNL2_DEBUG_MODE >= NNL2_DEBUG_MODE_VERBOSE
        NNL2_FUNC_ENTER();
    #endif
    ...
    if(total_elems < NNL2_MIN_MINF_INPLACE_PARALLEL_THRESHOLD) {
        naive_min_minf_inplace(tensor, threshold);
        #if NNL2_DEBUG_MODE >= NNL2_DEBUG_MODE_VERBOSE
            NNL2_FUNC_EXIT();
        #endif
        return;
    ...
    // Warning for unaligned memory in safety modes
        ...
        if(!is_aligned) {
            NNL2_WARN("In nnl2_own_min_minf_inplace, tensor memory is not aligned to 32 bytes. Performance may be suboptimal");
        }
    #endif
    
    size_t num_threads = NNL2_NUM_THREADS;
    pthread_t threads[num_threads];
    min_minf_inplace_ptask tasks[num_threads];
    ...
    for (size_t i = 0; i < num_threads; i++) {
        ...
        switch(dtype) {
            case FLOAT64: worker_func = nnl2_own_pmin_minf_inplace_float64; break;
            case FLOAT32: worker_func = nnl2_own_pmin_minf_inplace_float32; break;
			case INT64:   worker_func = nnl2_own_pmin_minf_inplace_int64;   break;
            case INT32:   worker_func = nnl2_own_pmin_minf_inplace_int32;   break;
        ...
        if(status != 0) {
            NNL2_THREAD_CREATE_ERROR(status, "nnl2_own_min_minf_inplace");
            num_threads = i;
            break;
        }
    ...
    for (size_t i = 0; i < num_threads; i++) {
        ...
        if(join_status != 0) {
            NNL2_THREAD_JOIN_ERROR(join_status, "nnl2_own_min_minf_inplace");
        }
    }
 ...
 **/
void* nnl2_own_pmin_minf_inplace_float64(void* arg) {
    min_minf_inplace_ptask* task = (min_minf_inplace_ptask*)arg;
 ...
 * See documentation at declaration
 * 
 ** @see nnl2_own_pmin_minf_inplace_float32
 **/
void* nnl2_own_pmin_minf_inplace_float32(void* arg) {
    min_minf_inplace_ptask* task = (min_minf_inplace_ptask*)arg;
 ...
 * See documentation at declaration
 * 
 ** @see nnl2_own_pmin_minf_inplace_int64
 **/
void* nnl2_own_pmin_minf_inplace_int64(void* arg) {
    min_minf_inplace_ptask* task = (min_minf_inplace_ptask*)arg;
    int64_t* data = (int64_t*)task->tensor->data;
    size_t start = task->start;
    size_t end = task->end;
    ...
    // Scalar processing for remainder
    for(; i < end; i++) {
        data[i] = (data[i] < threshold) ? data[i] : threshold;
    }
 ...
 * See documentation at declaration
 * 
 ** @see nnl2_own_pmin_minf_inplace_int32
 **/
void* nnl2_own_pmin_minf_inplace_int32(void* arg) {
    min_minf_inplace_ptask* task = (min_minf_inplace_ptask*)arg;
 ...
 */
nnl2_runtime_implementation min_minf_inplace_backends[] = {
    REGISTER_BACKEND(naive_min_minf_inplace, nnl2_naive, NAIVE_BACKEND_NAME),
        ...
        REGISTER_BACKEND(nnl2_own_min_minf_inplace, nnl2_own, NNL2_OWN_NAME),
    #endif
};    

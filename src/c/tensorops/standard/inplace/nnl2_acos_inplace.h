...
void nnl2_naive_acos_inplace(nnl2_tensor* tensor) {
	...
	// Additional checks at the maximum safety level
        ...
        NNL2_CHECK_NULL_IF_ERR_RETURN(tensor, "Tensor is NULL (.acos!)");
        NNL2_CHECK_NULL_IF_ERR_RETURN(tensor->data, "Tensor's data is NULL (.acos!)");
	#endif
	...
	switch(tensor->dtype) {
		...
		case INT32: {
            ...
            // Element-wise acos
            for(size_t i = 0; i < numel; i++) {
                tensor_data[i] = (nnl2_int32)acos((double)tensor_data[i]);
            }
 ...
 */
Implementation acosinplace_backends[] = {

    ...
    if(typea == typeb) {
        ...
        switch(typea) {
			...
			case INT64: {
				...
				for(size_t i = 0; i < total_elems; i++) {
					void* elem_b = data_b + i * typeb_step;
					data_a[i] = MAX(data_a[i], nnl2_convert_to_int64(elem_b, typeb));
				}
				break;
			}
...
/** @brief
 * Worker function for parallel 64-bit integer in-place maximum operation
 * 
 ** @param arg 
 * Pointer to maxinplace_ptask structure containing thread parameters
 *
 ...
 ** @see nnl2_own_pmaxinplace_float64
 **/
void* nnl2_own_pmaxinplace_int64(void* arg);
...
void nnl2_own_maxinplace(nnl2_tensor* tensora, const nnl2_tensor* tensorb) {
    ...
    for (size_t i = 0; i < num_threads; i++) {
        ...
        switch(dtype_a) {
			...
			case INT64:   worker_func = nnl2_own_pmaxinplace_int64;   break;
 ...
 * See documentation at declaration
 * 
 ** @see nnl2_own_pmaxinplace_int64
 **/
void* nnl2_own_pmaxinplace_int64(void* arg) {
    maxinplace_ptask* task = (maxinplace_ptask*)arg;
    ...
    if(task->aligned) {
        for(; i + 3 < end; i += 4) {
            ...
            // For 64-bit integers, use compare and blend

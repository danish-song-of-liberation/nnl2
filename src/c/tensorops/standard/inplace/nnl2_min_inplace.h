...
/** @brief
 * Threshold for enabling parallel execution of in-place minimum operation
 */
#define NNL2_MIN_INPLACE_PARALLEL_THRESHOLD 1000000

/** @brief
 * Worker function for parallel double precision in-place minimum operation
 * 
 ** @param arg 
 ...
 */
void* nnl2_own_pmininplace_float64(void* arg);

/** @brief
 * Worker function for parallel single precision in-place minimum operation
 * 
 ** @param arg 
 * Pointer to mininplace_ptask structure containing thread parameters  
 * 
 ...
 **/
void* nnl2_own_pmininplace_float32(void* arg);

/** @brief
 * Worker function for parallel integer in-place minimum operation
 * 
 ** @param arg 
 * Pointer to mininplace_ptask structure containing thread parameters
 *
...
void nnl2_own_mininplace(Tensor* tensora, const Tensor* tensorb) {
    ...
    if(total_elems < NNL2_MIN_INPLACE_PARALLEL_THRESHOLD || dtype_a != dtype_b) {
        naive_mininplace(tensora, tensorb);
        #if NNL2_DEBUG_MODE >= NNL2_DEBUG_MODE_VERBOSE
            NNL2_FUNC_EXIT();
        #endif
        return;
    ...
    // Warning for unaligned memory in safety modes
        ...
        if(!is_aligned) {
            NNL2_WARN("In nnl2_own_mininplace, tensor memory is not aligned to 32 bytes. Performance may be suboptimal");
        }
    #endif
    
    size_t num_threads = NNL2_NUM_THREADS;
    pthread_t threads[num_threads];
    mininplace_ptask tasks[num_threads];
    ...
    for (size_t i = 0; i < num_threads; i++) {
        ...
        if(status != 0) {
            NNL2_THREAD_CREATE_ERROR(status, "nnl2_own_mininplace");
            num_threads = i;
            break;
        }
    ...
    for (size_t i = 0; i < num_threads; i++) {
        ...
        if(join_status != 0) {
            NNL2_THREAD_JOIN_ERROR(join_status, "nnl2_own_mininplace");
        }
    }
 ...
 * See documentation at declaration
 * 
 ** @see nnl2_own_pmininplace_float32
 **/
void* nnl2_own_pmininplace_float32(void* arg) {
    mininplace_ptask* task = (mininplace_ptask*)arg;
 ...
 * See documentation at declaration
 * 
 ** @see nnl2_own_pmininplace_int32
 **/
void* nnl2_own_pmininplace_int32(void* arg) {
    mininplace_ptask* task = (mininplace_ptask*)arg;
...
Implementation mininplace_backends[] = {
	REGISTER_BACKEND(naive_mininplace, nnl2_naive, NAIVE_BACKEND_NAME),
		...
		REGISTER_BACKEND(nnl2_own_mininplace, nnl2_own, NNL2_OWN_NAME),
	#endif
};

...
void nnl2_naive_asin_inplace(nnl2_tensor* tensor) {
	...
	// Additional checks at the maximum safety level
        ...
        NNL2_CHECK_NULL_IF_ERR_RETURN(tensor, "Tensor is NULL (.asin!)");
        NNL2_CHECK_NULL_IF_ERR_RETURN(tensor->data, "Tensor's data is NULL (.asin!)");
	#endif
	...
	switch(tensor->dtype) {
		...
		case INT32: {
            ...
            // Element-wise asin
            for(size_t i = 0; i < numel; i++) {
                tensor_data[i] = (nnl2_int32)asin((double)tensor_data[i]);
            }
 ...
 */
Implementation asininplace_backends[] = {

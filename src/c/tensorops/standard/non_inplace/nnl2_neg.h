...
/** @brief
 * Creates a new tensor with element-wise negation of the input tensor.
 * Each element is computed as: out[i] = -tensor[i]
 *
 ** @param tensor
 * Input tensor
 *
 ** @return
 * New tensor with negated values
 *
 ** @note
 ...
 * @see nnl2_product
 */
nnl2_tensor* nnl2_naive_neg(const nnl2_tensor* tensor) {
 ...
 * @brief Backend implementations for out-of-place neg operation
 * @details
 * Array follows the standard backend registration pattern.
 * Currently registered backends:
 *  - nnl2_naive_neg: Basic reference implementation
 *
 * @see nnl2_naive_neg
 */
...
/**
 * @brief Function pointer for out-of-place neg operation
 * @ingroup backend_system
 */
negfn nnl2_neg;
 ...
 */
MAKE_CURRENT_BACKEND(neg);

/**
 * @brief Sets the backend for out-of-place neg operation
 * @ingroup backend_system
 * @param backend_name Name of the backend to activate
 * @see SET_BACKEND_BY_NAME
 * @see ESET_BACKEND_BY_NAME
 */
void set_neg_backend(const char* backend_name) {
    ESET_BACKEND_BY_NAME(neg_backends, nnl2_neg, backend_name, CURRENT_BACKEND(neg));
}

/**
 * @brief Gets the name of the active backend for out-of-place neg operation
 * @ingroup backend_system
 * @return Name of the current backend
 * @see CURRENT_BACKEND
 */
const char* get_neg_backend() {
    return CURRENT_BACKEND(neg);
}

/**
 * @brief Function declaration for getting all `neg` available backends
 * @ingroup backend_system
 * @see DEFINE_GET_BACKENDS_FUNCTION
 */
DEFINE_GET_BACKENDS_FUNCTION(neg);

/**
 * @brief Function declaration for getting the number of all `neg` backends
 * @ingroup backend_system
 * @see DEFINE_GET_NUMS_BACKENDS_FUNCTION
 */
DEFINE_GET_NUMS_BACKENDS_FUNCTION(neg);

...
nnl2_tensor* nnl2_naive_asin(const nnl2_tensor* tensor) {
    ...
    // Safety checks at maximum safety level
        ...
        NNL2_CHECK_NULL_IF_ERR_RETURN_VAL(tensor, "Tensor is NULL (.asin)", NULL);
        NNL2_CHECK_NULL_IF_ERR_RETURN_VAL(tensor->data, "Tensor's data is NULL (.asin)", NULL);
    #endif
    ...
    switch (tensor->dtype) {
        ...
        case INT32: {
            ...
            for (size_t i = 0; i < total_elems; i++) dst[i] = (nnl2_int32)asin((double)src[i]);
            break;
        }

...
nnl2_tensor* nnl2_naive_sqrt(const nnl2_tensor* tensor) {
    ...
    // Safety checks at maximum safety level
        ...
        NNL2_CHECK_NULL_IF_ERR_RETURN_VAL(tensor, "Tensor is NULL (.sqrt)", NULL);
        NNL2_CHECK_NULL_IF_ERR_RETURN_VAL(tensor->data, "Tensor's data is NULL (.sqrt)", NULL);
    #endif

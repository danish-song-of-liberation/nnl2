 ...
 */
nnl2_tensor* naive_max_broadcasting(nnl2_tensor* x, nnl2_tensor* y) {
...
/** @brief
 * Worker function for parallel int64 broadcasting maximum operation
 * 
 ** @param arg 
 * Pointer to max_broadcasting_ptask structure containing thread parameters
 *
 ...
 ** @see nnl2_own_pmax_broadcasting_float64
 **/
void* nnl2_own_pmax_broadcasting_int64(void* arg);
 ...
 */
nnl2_tensor* nnl2_own_max_broadcasting(const nnl2_tensor* x, const nnl2_tensor* y) {
    #if NNL2_DEBUG_MODE >= NNL2_DEBUG_MODE_VERBOSE
        NNL2_FUNC_ENTER();
    #endif
    ...
    if(numel_x < NNL2_MAX_BROADCASTING_PARALLEL_THRESHOLD || 
       x_dtype != y_dtype || 
       numel_y == 0) {
        nnl2_tensor* naive_result = naive_max_broadcasting((nnl2_tensor*)x, (nnl2_tensor*)y);
    ...
    for (size_t i = 0; i < num_threads; i++) {
        ...
        switch(x_dtype) {
            ...
            case INT64:   worker_func = nnl2_own_pmax_broadcasting_int64;   break;
            default: {
                NNL2_TYPE_ERROR(x_dtype);
                nnl2_free_tensor(result);
                #if NNL2_DEBUG_MODE >= NNL2_DEBUG_MODE_VERBOSE
 ...
 * See documentation at declaration
 * 
 ** @see nnl2_own_pmax_broadcasting_int64
 **/
void* nnl2_own_pmax_broadcasting_int64(void* arg) {
    max_broadcasting_ptask* task = (max_broadcasting_ptask*)arg;
 ...
 */
nnl2_runtime_implementation max_broadcasting_backends[] = {

#ifndef NNL2_BROADCASTING_MIN_INPLACE_H
#define NNL2_BROADCASTING_MIN_INPLACE_H
...
/** @brief
 * Threshold for enabling parallel execution of broadcasting minimum in-place operation
 */
#define NNL2_MIN_BROADCASTING_INPLACE_PARALLEL_THRESHOLD 100000

/** @brief
 * Worker function for parallel double precision broadcasting minimum in-place operation
 * 
 ** @param arg 
 * Pointer to min_broadcasting_inplace_ptask structure containing thread parameters
 *
 ** @return 
 ...
 */
void* nnl2_own_pmin_broadcasting_inplace_float64(void* arg);

/** @brief
 * Worker function for parallel single precision broadcasting minimum in-place operation
 * 
 ** @param arg 
 * Pointer to min_broadcasting_inplace_ptask structure containing thread parameters  
 * 
 ...
 * NULL (for pthread API compatibility)
 * 
 ** @see nnl2_own_pmin_broadcasting_inplace_float64
 **/
void* nnl2_own_pmin_broadcasting_inplace_float32(void* arg);

/** @brief
 * Worker function for parallel int64 broadcasting minimum in-place operation
 * 
 ** @param arg 
 * Pointer to min_broadcasting_inplace_ptask structure containing thread parameters
 *
 ...
 ** @see nnl2_own_pmin_broadcasting_inplace_float64
 **/
void* nnl2_own_pmin_broadcasting_inplace_int64(void* arg);

/** @brief
 * Worker function for parallel integer broadcasting minimum in-place operation
 * 
 ** @param arg 
 * Pointer to min_broadcasting_inplace_ptask structure containing thread parameters
 *
 ...
 ** @see nnl2_own_pmin_broadcasting_inplace_float64
 **/
void* nnl2_own_pmin_broadcasting_inplace_int32(void* arg);

/** @brief
 * High-performance parallel implementation of broadcasting element-wise minimum in-place operation
 * 
 ** @param x 
...
void nnl2_own_min_broadcasting_inplace(nnl2_tensor* x, const nnl2_tensor* y) {
    ...
    if(numel_x < NNL2_MIN_BROADCASTING_INPLACE_PARALLEL_THRESHOLD || 
       x_dtype != y_dtype || 
       numel_y == 0) {
        naive_min_broadcasting_inplace(x, y);
        #if NNL2_DEBUG_MODE >= NNL2_DEBUG_MODE_VERBOSE
            NNL2_FUNC_EXIT();
        #endif
        return;
    ...
    // Warning for unaligned memory in safety modes
        ...
        if(!is_aligned) {
            NNL2_WARN("In nnl2_own_min_broadcasting_inplace, tensor memory is not aligned to 32 bytes. Performance may be suboptimal");
        }
    #endif
    ...
    min_broadcasting_inplace_ptask tasks[num_threads];
    ...
    for (size_t i = 0; i < num_threads; i++) {
        ...
        switch(x_dtype) {
            case FLOAT64: worker_func = nnl2_own_pmin_broadcasting_inplace_float64; break;
            case FLOAT32: worker_func = nnl2_own_pmin_broadcasting_inplace_float32; break;
            case INT32:   worker_func = nnl2_own_pmin_broadcasting_inplace_int32;   break;
            case INT64:   worker_func = nnl2_own_pmin_broadcasting_inplace_int64;   break;
            default: {
                NNL2_TYPE_ERROR(x_dtype);
                #if NNL2_DEBUG_MODE >= NNL2_DEBUG_MODE_VERBOSE
                    NNL2_FUNC_EXIT();
        ...
        if(status != 0) {
            NNL2_THREAD_CREATE_ERROR(status, "nnl2_own_min_broadcasting_inplace");
            num_threads = i;
            break;
        }
    ...
    for (size_t i = 0; i < num_threads; i++) {
        ...
        if(join_status != 0) {
            NNL2_THREAD_JOIN_ERROR(join_status, "nnl2_own_min_broadcasting_inplace");
        }
    }
 ...
 **/
void* nnl2_own_pmin_broadcasting_inplace_float64(void* arg) {
    min_broadcasting_inplace_ptask* task = (min_broadcasting_inplace_ptask*)arg;
    ...
    for(size_t block = start_block; block < end_block; block++) {
        ...
        for(; i < block_size; i++) {
            x_data[block_offset + i] = MIN(x_data[block_offset + i], y_data[i]);
        }
    }
 ...
 * See documentation at declaration
 * 
 ** @see nnl2_own_pmin_broadcasting_inplace_float32
 **/
void* nnl2_own_pmin_broadcasting_inplace_float32(void* arg) {
    min_broadcasting_inplace_ptask* task = (min_broadcasting_inplace_ptask*)arg;
 ...
 * See documentation at declaration
 * 
 ** @see nnl2_own_pmin_broadcasting_inplace_int64
 **/
void* nnl2_own_pmin_broadcasting_inplace_int64(void* arg) {
    min_broadcasting_inplace_ptask* task = (min_broadcasting_inplace_ptask*)arg;
 ...
 * See documentation at declaration
 * 
 ** @see nnl2_own_pmin_broadcasting_inplace_int32
 **/
void* nnl2_own_pmin_broadcasting_inplace_int32(void* arg) {
    min_broadcasting_inplace_ptask* task = (min_broadcasting_inplace_ptask*)arg;
...
nnl2_runtime_implementation min_broadcasting_inplace_backends[] = {
    REGISTER_BACKEND(naive_min_broadcasting_inplace, nnl2_naive, NAIVE_BACKEND_NAME),
        ...
        REGISTER_BACKEND(nnl2_own_min_broadcasting_inplace, nnl2_own, NNL2_OWN_NAME),
    #endif
};  
...
#endif /** NNL2_BROADCASTING_MIN_INPLACE_H **/
